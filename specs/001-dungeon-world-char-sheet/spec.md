...
- **FR-007**: System MUST persist a single character's data to browser local storage so that refreshing the page retains all entered data. Only one character is supported; editing replaces the existing character. Changes MUST be saved automatically on field blur (when user leaves a field). **CRITICAL**: If a field is invalid, the save is blocked and an inline error message displays below the field until the user corrects the value. Valid data only is persisted. **Implementation Note**: When any field is blurred, the entire form state is saved to localStorage, excluding only those specific fields that currently have validation errors. This ensures consistent persistence behavior and prevents partial state issues.
